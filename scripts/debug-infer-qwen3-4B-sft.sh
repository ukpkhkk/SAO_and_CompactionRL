...
SFT_TORCH_DIST_DIR="${SFT_TORCH_DIST_DIR:-/mnt/workspace/models/font-info/qwen3-4b-sft_torch_dist}"
LOG_DIR="${LOG_DIR:-${PROJECT_DIR}/qwen3_4b_vanillappo_checkpoints}"
EXP_NAME="${EXP_NAME:-qwen3-4b-sft-megatron-update-debug-infer}"
...
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${LOG_DIR}/${EXP_NAME}}"
...
if [ ! -d "${MODEL_DIR}" ]; then
   echo "Cannot find SFT HF checkpoint dir: ${MODEL_DIR}" >&2
   exit 1
fi

if [ ! -f "${SFT_TORCH_DIST_DIR}/latest_checkpointed_iteration.txt" ]; then
   echo "Cannot find SFT Megatron checkpoint tracker: ${SFT_TORCH_DIST_DIR}/latest_checkpointed_iteration.txt" >&2
   echo "Set SFT_TORCH_DIST_DIR=/path/to/qwen3-4b-sft_torch_dist if your checkpoint is elsewhere." >&2
   exit 1
fi
...
CKPT_ARGS=(
   ...
   --ref-load "${SFT_TORCH_DIST_DIR}"
   --load "${SFT_TORCH_DIST_DIR}"
   --no-load-optim
   --no-load-rng
   --no-save-optim
)
